...
| 観点 | Google Search | RAG Engine | Vertex AI Search |
|-----|--------------|------------|-----------------|
| データソース | 公開 Web | 社内・専有データ | 社内・Web・構造化の横断 |
| 鮮度 | リアルタイム | インポート時点 | インポート / クロール時点 |
| セキュリティ | 公開情報のみ | VPC 内完結可 | VPC 内完結可 |
| セットアップ | ゼロ（API 有効化のみ） | コーパス構築が必要 | Datastore + Engine 構築 |
| 検索機能 | Google 検索品質 | ベクトル類似度 | Google 品質IR＋ブースト・ファセット |
| 向いている場面 | 最新 Web 情報の参照 | チャットボット・QA | 社内ポータル・EC 検索 |

→ Vertex AI Search の詳細は `_study/vertex-ai-search.md` を参照。
